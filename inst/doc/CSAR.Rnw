...
We will use a dataset included in the \CSAR package for this demonstration. The data represent a subset of the sequenced reads for a SEP3 ChIP-seq experiment \cite{Kaufmann2009} in $Arabidopsis$.

First, we load the \CSAR package and the data \Rclass{CSAR-dataset}. We run the \Rclass{mappedReads2Nhits} function to calculate the number of hits (number of extended reads that overlap a particular position) per nucleotide position for the control and sample dataset. See function \Rclass{mappedReads2Nhits} for more information about the parameter values.
...
nhitsS<-mappedReads2Nhits(sampleSEP3_test,chr=c("CHR1v01212004"),chrL=c(100000))
nhitsC<-mappedReads2Nhits(controlSEP3_test,chr=c("CHR1v01212004"),chrL=c(100000))
@

After, we run the function \Rclass{ChIPseqScore} to calculate the read-enrichment score of the sample compared to the control for each nucleotide position. \Rclass{sigWin} will generate candidate read-enriched regions, and \Rclass{score2wig} will generate a wig file that can be read by standard genome browsers.
...
We obtain with eah run of the function \Rclass{permutatedWinScores} one set of permutated scores. Later,we can get the distribution of score values after permutation with the function \Rclass{getPermutatedWinScores}.
